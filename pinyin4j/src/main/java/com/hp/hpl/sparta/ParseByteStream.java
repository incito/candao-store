...
class ParseByteStream implements ParseSource{
     ...
     */
    public ParseByteStream(String systemId,
                           InputStream istream,
                           ParseLog log,
        ...
        throws ParseException, IOException
    {
        ...
        try{
            ...
            InputStreamReader reader
                = new InputStreamReader(istream, fixEncoding(guessedEncoding));
            try{

                parseSource_ = new ParseCharStream( systemId,
                                                    reader,
                                                    log,
                                                    guessedEncoding,
                                                    handler);
            //}catch( CharConversionException e ){
                ...
                log.note("Problem reading with assumed encoding of "
                         +guessedEncoding
                         +" so restarting with "+secondGuessEncoding,
                         systemId,1);
                istream.reset();
                try{
     ...
     *  . */
    static private String guessEncoding(
                                        String systemId,
                                        byte[] start,
                                        int n,
                                        ParseLog log
        ...
        throws IOException
    {
        ...
        if( n != 4 ){
            ...
            encoding = "UTF-8";
        }else if( equals(   start, 0x0000FEFF)
                  || equals(start, 0xFFFE0000)
                  || equals(start, 0x0000FFFE)
                  || equals(start, 0xFEFF0000)
                  || equals(start, 0x0000003C)
                  || equals(start, 0x3C000000)
                  || equals(start, 0x00003C00)
                  || equals(start, 0x003C0000)
                  )
            encoding = "UCS-4";
    ...
    static private String hex(byte b){
        ...
        case 1:  return "0"+s;
        case 2:  return s;
        default: return s.substring(s.length()-2);
        }
    }
