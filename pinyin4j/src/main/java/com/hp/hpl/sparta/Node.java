...
public abstract class Node{
    ...
    void replaceInLinkedList(Node replacement) {
        if (previousSibling_ != null)
            previousSibling_.nextSibling_ = replacement;
        if (nextSibling_ != null)
            nextSibling_.previousSibling_ = replacement;
    ...
    */
    public boolean xpathSetStrings(String xpath, String value)
        ...
        try {
            ...
            if (xpath.charAt(slash + 1) == '@') {
                ...
                while (i.hasMoreElements()) {
                    ...
                    for (Node j = parentOfText.getFirstChild();
                        j != null;
                        j = j.getNextSibling())
    ...
    /** Select the first element that matches the relative XPath
        ...
        there is no match.*/
    public abstract Element xpathSelectElement(String xpath)
        throws ParseException;
    ...
    static protected void htmlEncode(Writer writer, String string)
        throws IOException {
        int n = string.length();
        int writeNext = 0;
        for (int i = 0; i < n; ++i) {
