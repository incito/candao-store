...
class ParseCharStream implements ParseSource {
    ...
    public ParseCharStream(
        ...
        */
        ...
        if (docTypeName_ != null
            && !docTypeName_.equals(rootElement.getTagName()))
            log_.warning(
                "DOCTYPE name \""
                    + docTypeName_
                    + "\" not same as tag name, \""
    ...
    */
    ...
    private final void readChar(char expected)
    ...
    private final boolean isChar(char expected)
        throws ParseException, IOException {
        if (curPos_ >= endPos_)
            if (fillBuf() == -1)
                throw new ParseException(this, "unexpected end of expression.");
    ...
    private final char readChar(char expect0, char expect1)
        throws ParseException, IOException {
        final char ch = readChar();
    ...
    private final char readChar(
        ...
        if (ch != expect0 && ch != expect1 && ch != expect2 && ch != expect3)
            throw new ParseException(
                this,
                ch,
                new char[] { expect0, expect1, expect2, expect3 });
        return ch;
    }

    private final boolean isChar(char expect0, char expect1)
        throws ParseException, IOException {
        if (curPos_ >= endPos_)
            if (fillBuf() == -1)
                return false;
    ...
    */
    ...
    static private boolean isLetter(char ch) {
        return "abcdefghijklmnopqrstuvwxyz".indexOf(Character.toLowerCase(ch))
            != -1;
    }
    ...
    */
    ...
    private void readXmlDecl()
        ...
        if (isEncodingDecl()) {
            ...
            if (encoding_ != null
                && !encodingDeclared.toLowerCase().equals(encoding_))
                throw new EncodingMismatchException(
                    systemId_,
                    encodingDeclared,
                    encoding_);
        }
    ...
    */
    private void readMarkupDecl() throws ParseException, IOException {
        if (isPi())
            ...
            isSymbol(MARKUPDECL_BEGIN)) { // (element-|Attlist-|Entity-|Notation-)Decl
            while (!isChar('>')) {
                if (isChar('\'', '\"')) {
                    char quote = readChar();
                    while (!isChar(quote))
    ...
    private char readCharRef() throws ParseException, IOException {
        ...
        while (!isChar(';')) {
            ...
            if (i >= TMP_BUF_SIZE) {
                log_.warning(
                    "Tmp buffer overflow on readCharRef",
                    systemId_,
                    getLineNumber());
                return ' ';
            }
    ...
    */
    ...
    private final void readSymbol(char[] expected)
        throws ParseException, IOException {
        ...
        for (int i = 0; i < n; ++i) {
            if (H_DEBUG)
                history_.addChar(cbuf_[curPos_ + i]);

            if (cbuf_[curPos_ + i] != expected[i])
                throw new ParseException(
                    this,
                    new String(cbuf_, curPos_, n),
                    expected);
        }
    ...
    private final boolean isSymbol(char[] expected)
        throws ParseException, IOException {
