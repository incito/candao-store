...
class XPathVisitor implements Visitor {
            ...
            if (node instanceof Element) {
                ...
                for (Node n = element.getFirstChild();
                    ...
                    if (n instanceof Text)
                        nodelistRaw_.add(((Text) n).getData());
            }
        }
    }
    ...
    private void accumulateMatchingElements(
        Document document,
        String tagName) {
    ...
    public void visit(TextExistsExpr a) throws XPathException {
        if (!(node_ instanceof Element))
            throw new XPathException(
                ...
                "Cannot test attribute of document");
        Element element = (Element) node_;
        for (Node i = element.getFirstChild();
            i != null;
            i = i.getNextSibling()) {
    ...
    public Element getFirstResultElement() {
        return nodelistFiltered_.size() == 0
            ? null
            : (Element) nodelistFiltered_.elementAt(0);
    }
    ...
    public String getFirstResultString() {
        return nodelistFiltered_.size() == 0
            ? null
            : (String) nodelistFiltered_.elementAt(0).toString();
    }
