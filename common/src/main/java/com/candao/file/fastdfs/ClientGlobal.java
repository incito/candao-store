...
package com.candao.file.fastdfs;
...
import com.candao.file.common.MyException;
...
{
	public static int g_connect_timeout = 5000; //millisecond
	public static int g_network_timeout = 30000; //millisecond
...
//  		iniReader = new IniFileReader(conf_filename);
  		g_connect_timeout = Integer.parseInt(Constant.TRACKERSERVER_CONNET_TIMEOUT == null?"5":Constant.TRACKERSERVER_CONNET_TIMEOUT) * 1000;
  		 
  		// g_connect_timeout  = g_connect_timeout * 1000; //millisecond
  		
  		g_network_timeout = Integer.parseInt(Constant.TRACKERSERVER_NETWORK_TIMEOUT == null?"30" : Constant.TRACKERSERVER_NETWORK_TIMEOUT) * 1000;
  		 
  		//g_network_timeout  = g_network_timeout * 1000; //millisecond

  		g_charset = Constant.TRACKERSERVER_CHARSET;
  		if (g_charset == null || g_charset.length() == 0)
  		{
  			g_charset = "ISO8859-1";
  		}
  		
  		szTrackerServers = Constant.TRACKERSERVER.split(",");
  		...
  		g_tracker_http_port = Integer.parseInt(Constant.TRACKERSERVER_HTTP_PORT == null ?"80":Constant.TRACKERSERVER_HTTP_PORT);
  		g_anti_steal_token = Boolean.parseBoolean(Constant.TRACKERSERVER_HTTP_PORT == null ?"false":Constant.TRACKERSERVER_HTTP_PORT);
...
//	public static void initProp(String conf_filename) throws FileNotFoundException, IOException, MyException
//	{
...
//		sock.connect(new InetSocketAddress(ip_addr, port), 3000);
		return sock;
	}
