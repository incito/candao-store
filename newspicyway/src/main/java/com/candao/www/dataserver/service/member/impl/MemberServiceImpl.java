...
/**
 * 雅座会员业务类 Created by lenovo on 2016/3/21.
 */
...
public class MemberServiceImpl implements MemberService {
	...
	@Autowired
	@Qualifier("candaoMemberHandler")
	private MemberHandler candaoMemberHandler;
	 ...
	 * @param orderId
	 * @param isRevert2
	 *            true:setMemberPrice2 false:setMemberPrice3
	 * @return
	 */
	...
	@Override
	public String StoreCardDeposit(String memberTrans) {
		MemberHandler memberHandler = getMemberHandler();
		try {
			if (null != memberHandler) {
				return memberHandler.storeCardDeposit(memberTrans);
			}
		} catch (Exception e) {
			logger.error("->", e);
			return memberHandler.errorResult(memberTrans, "服务器发生异常");
		}
		return null;
	}

	private MemberHandler getMemberHandler() {
		PadConfig configinfos = padConfigDao.getconfiginfos();
		if ("1".equals(configinfos.getViptype())) {
			candaoMemberHandler.init(configinfos);
			return candaoMemberHandler;
		} else if ("2".equals(configinfos.getViptype())) {
