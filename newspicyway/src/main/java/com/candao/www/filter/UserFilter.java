...
public class UserFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain chain) throws IOException, ServletException {
		...
		if (!"/".equals(StringUtils.left(targetURL, 6))) {
			if (!"/login".equals(StringUtils.left(targetURL, 6)) && ! targetURL.startsWith("/padinterface")&& ! targetURL.startsWith("/bankinterface")&& ! targetURL.startsWith("/member")&& ! targetURL.startsWith("/weixin")
