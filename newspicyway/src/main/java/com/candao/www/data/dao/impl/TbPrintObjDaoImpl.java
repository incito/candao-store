...
public class TbPrintObjDaoImpl implements TbPrintObjDao {
    ...
    public int insertPrintDishBatch(List<PrintDish> printDishs) {
        return dao.insert(PREFIX + ".insertPrintDishBatch", printDishs);
    }

    @Override
    public int insertPrintObj(PrintObj printObj) {
        return dao.insert(PREFIX + ".insertPrintObj", printObj);
    }

    @Override
    public int updateTimemsg(String timemsg, String id) {
        Map<String, Object> paramMap = new HashMap<>();
        paramMap.put("timemsg", timemsg);
        paramMap.put("id", id);
        return dao.update(PREFIX + ".updateTimemsg", paramMap);
    }
}
