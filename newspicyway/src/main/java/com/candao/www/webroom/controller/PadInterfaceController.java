...
public class PadInterfaceController {
	 ...
	 *            POS登录 1 POS 开业 2 POS 反结算 3 POS 清机 4 POS 结业 5 POS 收银 6 POS 退菜 7
	 *            PAD 开台 8 PAD
...
//				}
					...
					if (null != ttellerCashs) {
						return JacksonJsonMapper.objectToJson(ReturnMap.getFailureMap("您已在其他POS上登录，请在已登录POS上进行清机。清机完成后即可在本机登录。"));
					}
				}
		...
		public void run() {
			...
			if (tableList != null && tableList.size() > 0) {
				...
				if (torder != null && torder.getUserid() != "") {
					...
					String tableNo = null;
					try {
						areaname = java.net.URLEncoder.encode(String.valueOf(tableList.get(0).get("areaname")),
								"utf-8");
					...
					if (!userid.equals("")) {
						messageinfo.append(userid + "|" + finalmsgType + "|" + finalcallStatus + "|" + areaname + "|"
								+ tableNo + "|" + IdentifierUtils.getId().generate().toString());
						// new TsThread(messageinfo.toString()).start();
						URL urlobj;
						try {
