...
//	  }
	...
	public String rebackSettleOrder(SettlementInfo settlementInfo) {
		...
		if(againSettleNums == null || againSettleNums.equals("0")){
			...
			settlementMapper.insertSettlementHistory(orderId,settlementInfo.getReason(),1,settlementInfo.getUserName());
		}else{
			//如果不是第一次反结算，修改反结算表反结算次数字段，每反结算一次加1
			int nums = Integer.parseInt(againSettleNums)+1;
			settlementMapper.updateSettlementHistory(orderId, nums, settlementInfo.getUserName());
	    }
		//删除原结算信息
