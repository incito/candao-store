...
public class OrderSettleServiceImpl implements OrderSettleService {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(OrderSettleServiceImpl.class);
	...
	public String settleOrder(SettlementInfo settlementInfo) {
		...
		for (SettlementDetail detail : settlementInfo.getPayDetail()) {
			...
			recordDetail.setSdetailid(IdentifierUtils.getId().generate()
					.toString());
			...
			if (String.valueOf(Constant.PAYWAY.PAYWAY_FREE).equals(
					detail.getPayWay())) {
				recordDetail.setPayamount(detail.getPayAmount());
		...
		// dish.setOrderNum(String.valueOf(new BigDecimal(dish.getOrderNum() ==
		// null?"0":dish.getOrderNum() ).subtract(new
		// BigDecimal(detail.getDishnum() == null?"0":detail.getDishnum()))));
	...
	@Override
	@Transactional
	public SettlementStrInfoDto setInitData(
			SettlementStrInfoDto settlementStrInfoDto, WxPayResult payResult) {
