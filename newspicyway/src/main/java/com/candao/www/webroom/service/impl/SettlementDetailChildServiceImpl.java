...
public class SettlementDetailChildServiceImpl implements SettlementDetailChildService {
	...
	public List<Map<String,Object>> querySettDetailList(Map<String,Object> params){
		...
		for(Map<String,Object> data : list){
			...
			String paywayName = (String)data.get("itemid");
			String cardType = (String)data.get("membercardno");
			String itemDesc = (String)data.get("itemDesc");
			if(StringUtils.isNotBlank(paywayName) && paywayName.equals("1")){
				if(StringUtils.isNotBlank(cardType) && cardType.equals("1")){
					itemDesc = itemDesc + "(刷工行卡)";
				}else{
					itemDesc = itemDesc + "(刷他行卡)";
				}
			}
			value.put("itemDesc", itemDesc);
	...
	public Map<String,Object> querySettList(Map<String,Object> params){
	    ...
	    if(StringUtils.isNotBlank(itemid) && itemid.equals("1")){
			if(StringUtils.isNotBlank(membercardno) && membercardno.equals("1")){
				payWayName = payWayName + "(刷工行卡)";
			}else{
				payWayName = payWayName + "(刷他行卡)";
			}
		}
