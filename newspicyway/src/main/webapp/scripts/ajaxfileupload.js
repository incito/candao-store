...
//    alert(browser.version);
  ...
  {  
      ...
      if (data) {  
          for ( var i in data) {  
              $("<input  name='" + i + "' id='" + i + "' value='" + data[i] + "'  style='display:none;' />").appendTo(form);
          }  
      }  
//      if(fileElementId!=''&&fileElementId!='undefined'&&fileElementId!=undefined){
...
//      }
  ...
  ajaxFileUpload: function(s) {
    s = jQuery.extend({fileFilter:"",fileSize:2097152}, jQuery.ajaxSettings, s);
    //文件筛选
    ...
    if (s.fileSize > 0) {
      ...
      if (fs > s.fileSize) {
        alert("当前文件大小 (" +  Math.round(fs/1024/1024) + "M) 超过允许的限制值 (" + s.fileSize/1024/1024 +"M)！");
        return;
      }
    }
