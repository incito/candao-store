    ...
    <%@ page import="com.candao.www.constant.Constant" %>
    ...
    <%
		...
		File dir = new File(Constant.UPLOAD_PATH+"/"+filePath);
		if (!dir.exists()) {
			try {
				dir.mkdirs();
			} catch (Exception e) {
...
//     		String str = Constant.UPLOAD_PATH+"/"+filePath;
// 			File f = new File(str);
			String savePath = Constant.UPLOAD_PATH + "/"+filePath;
    		//格式验证
    		...
    		String saveName=random.nextInt(10000)
   	...
   	for(int i=0;i<outSrc.length;i++){
   		outstr+=request.getContextPath()+"/"+outSrc[i]+"ue_separate_ue";
   	}
