...
CREATE TABLE t_order_detail_history (
  ...
  couponid VARCHAR(50) DEFAULT NULL,
  disuserid VARCHAR(50) DEFAULT NULL,
  orignalprice DECIMAL(10, 2) DEFAULT NULL,
  pricetype VARCHAR(10) DEFAULT NULL,
  orderseq INT(11) DEFAULT NULL,
  relatedishid VARCHAR(4000) DEFAULT NULL,
  orderdetailid VARCHAR(255) NOT NULL DEFAULT '0',
  type INT(11) DEFAULT NULL COMMENT '0 并台 1 删除',
  ordertype INT(3) DEFAULT NULL COMMENT '0 单品 1 火锅 2 套餐',
  parentkey VARCHAR(255) DEFAULT NULL COMMENT '父类主键',
  superkey VARCHAR(255) DEFAULT NULL COMMENT '套餐的主键',
  ismaster INT(255) DEFAULT NULL COMMENT '是否是master 默认是 0 不是 1是',
  primarykey VARCHAR(255) DEFAULT NULL COMMENT '菜品主键',
  islatecooke INT(2) DEFAULT NULL COMMENT '0 否 1 是',
  isadddish INT(2) DEFAULT NULL COMMENT '0 否 1 是',
  childdishtype INT(2) DEFAULT NULL COMMENT '0 单品 1 火锅 2 套餐'
)
ENGINE = INNODB
...
CREATE TABLE t_order_history (
  ...
  gzamount DECIMAL(8, 2) DEFAULT 0.00 COMMENT '挂帐金额',
  ssamount DECIMAL(8, 2) DEFAULT 0.00 COMMENT '实收金额 现金，银行卡，会员卡，会员积分',
  ageperiod VARCHAR(255) DEFAULT NULL COMMENT '年龄段',
  type INT(11) DEFAULT NULL COMMENT '0 并台 1 删除'
)
ENGINE = INNODB
CHARACTER SET utf8
...
AS
	select `t_dictionary`.`dictid` AS `dictid`,`t_dictionary`.`itemid` AS `itemid`,`t_dictionary`.`itemDesc` AS `itemDesc`,`t_dictionary`.`itemSort` AS `itemSort`,`t_dictionary`.`status` AS `status`,`t_dictionary`.`type` AS `type`,`t_dictionary`.`typename` AS `typename`,`t_dictionary`.`begin_time` AS `begin_time`,`t_dictionary`.`end_time` AS `end_time`,`t_dictionary`.`charges_status` AS `charges_status`,`t_dictionary`.`member_price` AS `member_price`,`t_dictionary`.`price` AS `price`,`t_dictionary`.`date_type` AS `date_type`,`t_dictionary`.`item_value` AS `item_value` from `t_dictionary` where ((`t_dictionary`.`itemid` in ('0','1','5','8','13')) and (`t_dictionary`.`type` = 'PAYWAY'));
